...
fn main() {
    const MAIN_AREA_WIDTH: u16 = 65;
    const MAIN_AREA_HEIGHT: u16 = 33;
    ...
    app.run(|app_state: &mut State, window: &mut Window| {
        ...
        let square_size = Vec2::xy(MAIN_AREA_WIDTH - 1, MAIN_AREA_HEIGHT - 1);
        ...
        let right_x = left_x + square_size.x + 1;

        for key_event in app_state.keyboard().last_key_events() {
            match key_event {
                KeyEvent::Pressed(Key::Esc) => app_state.stop(),
                KeyEvent::Pressed(Key::W) => player.move_y(-1, MAIN_AREA_HEIGHT - 3),
                KeyEvent::Pressed(Key::S) => player.move_y(1, MAIN_AREA_HEIGHT - 3),
                KeyEvent::Pressed(Key::A) => player.move_x(-1, MAIN_AREA_WIDTH - 3),
                KeyEvent::Pressed(Key::D) => player.move_x(1, MAIN_AREA_WIDTH - 3),
                _ => (),
            }
        }
        ...
        draw_filled_rect(&mut pencil, '·', top_left, Vec2::xy(MAIN_AREA_WIDTH, MAIN_AREA_HEIGHT));
        pencil.set_foreground(Color::White);
        pencil.draw_rect(&RectCharset::double_lines(), top_left, Vec2::xy(MAIN_AREA_WIDTH, MAIN_AREA_HEIGHT));
        ...
        draw_ui_box(&mut pencil, "STATS" as &str, Vec2::xy(stats_x, top_left.y), Vec2::xy(sidebar_width, MAIN_AREA_HEIGHT), &stat_entries);

        // TODO Draw Inventory on the side using draw_ui_box()
        draw_ui_box(&mut pencil, "INVENTORY" as &str, Vec2::xy(right_x, top_left.y), Vec2::xy(sidebar_width, MAIN_AREA_HEIGHT), &vec!["IMAGINARY ITEM" as &str]);
